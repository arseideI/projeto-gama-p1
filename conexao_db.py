...
class ConexaoBD(object): #classe de conexão com o BD
    ...
    def __init__(self,Server,BD,Login,Senha,verbose, driver = "Driver={SQL Server}"): #método construtor, para receber os parâmetros de conexão.
        self.Server = Server #nome do servidor (local ou azure)
        self.BD = BD #nome do banco de dados 
        self.Login = Login #username do usuário
        self.Senha = Senha #password do usuário
        self.verbose = verbose #variável que setará as opções mysql ou sql server
        self.driver = driver #descrição do driver (linux ou windows) para sql server. Por padrão, utiliza-se windows.
        
    def cursor(self): #método responsável por integrar o comando conn.cursor do pyodbc na classe de conexão, para que posteriormente possa ser utilizado no código main.
    ...
    def conexao_azure(self): #aqui é feita a conexão com o bd, podendo ser mysql ou sqlserver, linux ou windows, local ou azure
        ...
        if self.verbose == "sqlserver": #opção que indica conexão com o sql server
            try:
                ...
                self.conn = pyodbc.connect(f"{self.driver};" #etapa de conexão com o servidor do banco de dados sql server
            ...
            try:
                print(f"Conexão com o banco de dados {self.verbose} iniciada...")
                self.conn = mysql.connector.connect(host=self.Server, #etapa de conexão com o servidor do banco de dados mysql
                                                    user=self.Login,
                                                    password=self.Senha,
                                                    database=self.BD)
                print("A conexão ocorreu com êxito!")
            ...
            print("SGBD não suportado. Por favor, selecione as opções sqlserver ou mysql.") #caso o usuário escolha algum SGBD diferente dos citados acima.
